...
def main():
    ...
    prefab = {}
# block data
    prefab["header"] = unpack(bin_file, "s", 4) # char[], 4 bytes
    prefab["version"] = unpack(bin_file, "I") # usigned int, 4 bytes
    prefab["size_x"] = unpack(bin_file, "H") # unsigned short, 2 bytes
    prefab["size_y"] = unpack(bin_file, "H") # unsigned short, 2 bytes
    prefab["size_z"] = unpack(bin_file, "H") # unsigned short, 2 bytes
    # uint32[x*y*z] rawBlockData
    prefab["layers"] = [rawBlockData] # [][][] next section of unpacking is a looped usgined int, 4 bytes "s" for the total size of xyz
    # byte[x*y*z] blockDensity
    prefab["blockDensity"] = [blockDensity] # [][][] char, 1 byte "s"
    # byte[x*y*z][2] blockDamageData
    prefab["blockDamageData"] = [blockDamageData][2] # {([],[2]), ([],[2]), ([],[2])} char, 1 byte "s"
#not sure about this next part
# texture data
    # int32 numberOfBytes
    prefab["numberOfBytes"] = 0 # int32, 4 bytes "i"
    # byte[numberOfBytes] data
    prefab["textureData"] = [] # [][][] char, 1 byte "s"
    # each block that has texture data, >0
    # long[blocksWithTextureCount] texture
    prefab["textureID"] = 0 # long, 4 bytes
    prefab
